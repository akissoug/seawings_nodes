...
class FaultDetector(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                # LIDAR parameters
                ('lidar_timeout', 10.0),
                ('min_lidar_range', 0.5),  # meters
                ('max_lidar_range', 50.0),  # meters
                ('enable_lidar_check', True),
        ...
        self.enable_lidar_check = self.get_parameter('enable_lidar_check').value
        ...
        # Rate limiting for GPS log messages (ONLY NEW ADDITION)
        ...
        if self.enable_sensor_checks:
            ...
            # LIDAR subscription
            if self.enable_lidar_check:
                self.create_subscription(
                    DistanceSensor, '/fmu/out/distance_sensor',
                    self.lidar_callback, qos_profile=px4_qos_profile
                )
    ...
    def print_startup_info(self):
        ...
        if self.sitl_mode:
            ...
            if self.enable_sensor_checks:
                ...
                if self.enable_lidar_check:
                    self.get_logger().info(f'  • LIDAR: range {self.min_lidar_range}-{self.max_lidar_range}m')
    ...
    def get_px4_mode_name(self, nav_state):
        """Get mode name from PX4 nav_state"""
        mode_map = {
            0: "MANUAL",
            1: "ALTITUDE",
            2: "POSITION",
    ...
    def gps_callback(self, msg):
        with self.data_lock:
            ...
            if gps_healthy:
                ...
                # Rate-limited logging for GPS issues (ONLY CHANGE IN THIS METHOD)
    ...
    def lidar_callback(self, msg):
        ...
        with self.data_lock:
            ...
            self.sensor_timestamps['lidar'] = time.time()
            
            # Check if LIDAR reading is valid
            if hasattr(msg, 'current_distance'):
                if msg.current_distance < self.min_lidar_range or msg.current_distance > self.max_lidar_range:
                    self.get_logger().warn(f'⚠️ LIDAR out of range: {msg.current_distance:.2f}m')
    ...
    def detect_mission_start(self, current_mode, previous_mode):
        """
        Unified mission start detection logic - MUST BE IDENTICAL ACROSS ALL NODES
    ...
    def check_sensor_health(self, current_time):
        ...
        if self.sensor_data['gps'] is None:
            # No GPS data at all
            if self.sitl_mode and (current_time - self.node_start_time) < 60:
                health_status['gps'] = True  # Grace period in SITL
            else:
                health_status['gps'] = False
                self.get_logger().warn('⚠️ No GPS data received')
        else:
            ...
            if self.sensor_timestamps['gps']:
                if (current_time - self.sensor_timestamps['gps']) > timeout:
                    gps_ok = False
                    self.get_logger().warn(f'⚠️ GPS timeout ({timeout}s)')
        ...
        if self.enable_sensor_checks and not self.sitl_mode:
            ...
            # LIDAR check
            if self.enable_lidar_check:
                if self.sensor_timestamps['lidar']:
                    lidar_ok = (current_time - self.sensor_timestamps['lidar']) < self.lidar_timeout
                    if lidar_ok and self.sensor_data['lidar']:
                        # Check if reading is in valid range
                        dist = self.sensor_data['lidar'].current_distance if hasattr(self.sensor_data['lidar'], 'current_distance') else 0
                        if dist < self.min_lidar_range or dist > self.max_lidar_range:
                            lidar_ok = False
                    health_status['lidar'] = lidar_ok
                else:
                    health_status['lidar'] = True  # Don't fail if no LIDAR
    ...
    def check_system_health(self):
        ...
        with self.data_lock:
            ...
            if not health_status.get('gps', True):
                ...
                self.get_logger().warn(f'🚨 GPS failure #{self.consecutive_gps_failures}/{failure_threshold}')
                ...
                if self.consecutive_gps_failures > 0:
                    self.get_logger().info(f'✅ GPS healthy again')
                self.consecutive_gps_failures = 0
            ...
            # Check for LIDAR failure (only in real craft)
            if not self.sitl_mode and self.enable_lidar_check:
                if not health_status.get('lidar', True):
                    if self.sensor_failure_counts['lidar'] > 6:
                        self.get_logger().warn('⚠️ LIDAR failure detected')
                        # Don't trigger RTL for LIDAR alone, just warn
    ...
    def trigger_emergency_rtl(self, reason):
        ...
        self.emergency_trigger_source = f"Fault Detector: {reason}"
